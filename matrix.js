...
const showResult2D = (title, containerId, dataArray) => {
    ...
    for (let i = 0; i < dataArray.length; i ++) {
        ...
        for (let ii = 0; ii < dataArray[0].length; ii ++) {
            let td = document.createElement('td');
            let span = document.createElement('span');

            span.innerHTML = dataArray[i][ii];
...
function performOperation(operation) {
    ...
    if (result.length === 0) {
        console.log("The result array is empty.");
    } else {
...
function addMatrices(matrix1, matrix2){ 
    ...
    for (let i = 0; i < matrix1.length; i ++) {
        result[i] = [];
        for (let ii = 0; ii < matrix1[0].length; ii ++) {
            result[i][ii] = matrix1[i][ii] + matrix2[i][ii];
        }
    }
...
const multiplyMatrices = (matrix1, matrix2) => { 
    ...
    for (let i = 0; i < matrix1.length; i ++) {
        ...
        for (let ii = 0; ii < matrix1.length; ii ++) {
            result[i][ii] = 0
            for (let iii = 0; iii < matrix2.length; iii ++) {
                result[i][ii] += matrix1[i][iii] * matrix2[iii][ii];
            }
        }
    }
